...
do
    ...
    psql -t -c "\COPY (SELECT county, 'https://www.openstreetmap.org/' || (CASE WHEN area_id < 0 THEN 'relation/' || (-area_id)::text ELSE 'way/' || area_id::text END) || '/' AS url, ROUND(area)::int AS area FROM landuse_export WHERE state = '$state' ORDER BY county, area DESC, url) TO STDOUT (FORMAT csv)" >> data/$state.csv
done
